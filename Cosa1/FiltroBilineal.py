...
for i in range(1, xx-1):
    for j in range(1, yy-1):
        a = escaleted_img1[i, j]
        b = escaleted_img1[i, j+1]
        c = escaleted_img1[i+1, j]
        d = escaleted_img1[i+1, j+1]
        f=escaleted_img1[i-1, j]
        g=escaleted_img1[i-1, j-1]
        h=escaleted_img1[i-1, j+1]
        k=escaleted_img1[i+1, j-1]
        l=escaleted_img1[i, j-1]
        m = (a + b + c + d+f+g+h+k+l) / 9
        escaleted_img1Filtro[i, j] = int(m)            
...
cv.imshow('Imagen Escalada (Filtro Bilineal)', escaleted_img1Filtro)
